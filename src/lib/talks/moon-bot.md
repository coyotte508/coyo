...
The result is **Moon Bot**: a Slack bot powered by the [Pi coding agent SDK](https://github.com/badlogic/pi-mono), running in a Kubernetes pod with privileged internal network access, and using **HuggingFace Buckets** as its persistent memory store.
...
The session file is an append-only JSONL trace of every turn: user messages, thinking blocks, tool calls, tool results, and assistant replies. HuggingFace renders these files natively as an [agent trace viewer](https://huggingface.co/changelog/agent-trace-viewer): just open the link and you can step through the full session turn by turn, seeing exactly what the model did, which tools it called, what they returned, and how it reasoned to its answer.

This makes every Moon Bot interaction fully auditable directly from Slack.

## Skills: pluggable domain knowledge

Skills are Markdown files in `skills/<name>/SKILL.md` following the [Agent Skills](https://agentskills.io) standard. They're injected into the system prompt and tell the LLM how to use specific tools or navigate specific codebases.
