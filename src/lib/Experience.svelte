...
<svelte:element
	...
	class="block overflow-hidden rounded-lg border border-neutral-200 bg-white shadow-sm transition-shadow hover:shadow-md"
...
>
	<div class="flex items-center gap-4 px-4 pt-4">
		{#if project.avatar || project.icon}
			<div
				class="flex h-12 w-12 shrink-0 items-center justify-center overflow-hidden rounded-full bg-neutral-50 text-2xl"
			>
				{#if project.avatar}
					{project.avatar}
		...
		<div>
			...
			<p class="text-sm text-neutral-500">{project.years}</p>
		</div>
	</div>
	...
	<div class="prose-sm px-4 py-4 text-neutral-700 [&_p]:mb-2">
